...
class Ui_Form(object):
        ...
        def update_account(self):
                ...
                if not self.check_username(username,self.currentName):
                        return
        ...
        def check_username(self, username, currentName):
                try:
                        data = mysql.connector.connect(
                        host="localhost",
                        user="root",
                        ...
                        if result == currentName:
                                QtWidgets.QMessageBox.about(None, "Error", "Username already exists")
                                return False
                        return True
                except Exception as e:
                        QtWidgets.QMessageBox.about(None, "Error", f"Error: {e}")
        ...
        def delete_account(self,currentPage):
                try:
                        ...
                        cursor.execute("DELETE FROM list_needs where iduser = %s", (self.iduser,))
                        cursor.execute("DELETE FROM list_wants where iduser = %s", (self.iduser,))
                        cursor.execute("DELETE FROM list_savings where iduser = %s", (self.iduser,))
