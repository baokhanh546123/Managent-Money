...
class Ui_Form(object):
    ...
    def search(self):
        list_name_needs = self.lineEdit.text()

        if not list_name_needs:
        ...
        try : 
            ...
            cursor = data.cursor()
            cursor.execute("""SELECT name_needs , cost  from list_needs where iduser = %s AND name_needs = %s
                           """,(self.iduser , list_name_needs,))
    ...
    def delete(self):
        ...
        try : 
            ...
            cursor = data.cursor()
            cursor.execute("SELECT COUNT(*) FROM list_needs")
            check = cursor.fetchone()

            if check[0] == 0 : 
                QMessageBox.warning(None , "Error" , "You need add data")
            else:
                cursor.execute("DELETE FROM list_needs WHERE iduser = %s AND name_needs = %s",(self.iduser , list_name_needs , ))
                data.commit()
