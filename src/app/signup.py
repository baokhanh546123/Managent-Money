...
class Ui_Form(object):
        ...
        def signup_button(self):
                ...
                if not username or not phone or not address or not fullname or not password or not co_password:
                        QtWidgets.QMessageBox.warning(None, "Warning", "Please fill out the form")
                elif len(password) < length:
                        QtWidgets.QMessageBox.warning(None, "Warning", "The password is too short")
                elif not any(char in special_characters for char in password):
                        QtWidgets.QMessageBox.warning(None, "Warning", "The password needs to have a special character")
                elif not phone.isdigit() and len(phone) == 10:
                        QtWidgets.QMessageBox.warning(None, "Warning", "Phone number must be 10 digits")
                elif password != co_password:
                        QtWidgets.QMessageBox.warning(None, "Warning", "Passwords do not match")
                        ...
                        try:
                                ...
                                if account_username == None : 
                                        ...
                                        if username == account_username : 
                                                QMessageBox.warning(None , "Error" , "Username already exitst")

                                insert_query = "INSERT INTO users (username, password, phone, address, fullname, times) VALUES (%s, %s, %s, %s, %s, NOW())"
                                cursor.execute(insert_query, (username, hashed_password, phone, address, fullname))
                                data.commit()
                                QtWidgets.QMessageBox.information(None, "Success", "Successfully signed up")
                                self.login(self.Form)

                        except mysql.connector.Error as error:
                                QMessageBox.warning(f"Failed {error}")
                        finally:
        ...
        def login(self, currentForm):
                from loginUi1 import Ui_Form as LoginForm
