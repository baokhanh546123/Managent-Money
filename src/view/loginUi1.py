...
class Ui_Form(object):
        ...
        def connect(self, username, hashed_password):
                try:
                        ...
                        cursor = data.cursor()
                        cursor.execute("SELECT iduser FROM users WHERE username = %s AND password = %s", (username, hashed_password))
                        user = cursor.fetchone()

                        if user:
                                self.current_iduser = user[0]
                                QtWidgets.QMessageBox.information(None, "Success", "Successfully logged in")
                                return self.current_iduser  
                        else:
                                QtWidgets.QMessageBox.warning(None, "Error", "Username or password incorrect")
                                return None  

                except mysql.connector.Error as error:
                        QtWidgets.QMessageBox.critical(None, "Error", f"Database error: {error}")
                        return None
        ...
        def clicked_login(self):
                ...
                user_id = self.connect(username, hashed_password)

                if user_id:
                        self.open_main(self.Form , user_id)
                        self.Form.close()

        def clicked_signup(self,currentForm):
                from view.signup import Ui_Form as SignUiForm
