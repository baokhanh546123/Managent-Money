...
from view.ex11 import Ui_Form as Main
import view.image
import view.image1
import sys 
...
"")
    ...
    def warning(self,checked = False):
        try:
            ...
            if needs:
                ...
                cursor.execute("SELECT SUM(cost) AS Total_cost FROM needs WHERE iduser = %s", (self.iduser,))
    ...
    def one(self):
        ...
        name_needs = self.check_name(self.needs_name)

        if not self.check_needs():
            return
        elif name == name_needs:
            QMessageBox.warning(None , "Error" , f"Name of {self.needs_name} have already exits.")
            return
        elif not name : 
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO needs (iduser , ten , cost , times ) VALUES (%s, %s , %s , NOW())", (self.iduser , name , 100000))
            data.commit()
    ...
    def two(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO needs (iduser , ten , cost , times ) VALUES (%s, %s , %s , NOW())", (self.iduser , name , 200000))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
            self.lineEdit.clear()
    ...
    def five(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO needs (iduser , ten , cost , times) VALUES (%s, %s , %s,NOW())", (self.iduser , name , 500000))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
            self.lineEdit.clear()
    ...
    def option(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO needs (iduser , ten , cost ,times) VALUES (%s, %s , %s,NOW())", (self.iduser , name , cost))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
