...
class Ui_Form(object):
    ...
    def search(self):
        ...
        try : 
            ...
            cursor = data.cursor()
            cursor.execute("SELECT ten , cost  from needs where iduser = %s and ten LIKE %s",(self.iduser , f"%{name_needs}%",))
            result = cursor.fetchone()
            if result is None :
                self.label_2.setText("No found data")
            else:
    ...
    def delete(self):
        ...
        try : 
            ...
            if check[0] == 0 : 
                ...
                cursor.execute("DELETE FROM needs WHERE iduser = %s AND ten = %s order by times limit 1 ",(self.iduser , name_needs , ))
                data.commit()
