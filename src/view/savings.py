...
"}")
    ...
    def one(self):
        ...
        name_savings = self.check_name(self.name_savings)
        if not self.check_savings():
            return 
        elif name == name_savings:
            QMessageBox.warning(None , "Error" , "Name of savings have already exits.")
            return
        elif not name : 
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO savings (iduser , ten , cost , times) VALUES (%s, %s , %s ,NOw())", (self.iduser , name , 1000000))
            data.commit()
    ...
    def two(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO savings (iduser , ten , cost , times) VALUES (%s, %s , %s ,NOW())", (self.iduser , name , 10000000))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
    ...
    def five(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO savings (iduser , ten , cost , times ) VALUES (%s, %s , %s , NOW())", (self.iduser , name , 100000000))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
    ...
    def option(self):
        ...
        try:
            data = self.get_db_connection()
            cursor = data.cursor()
            cursor.execute("INSERT INTO savings (iduser , ten , cost , times) VALUES (%s, %s , %s , NOW())", (self.iduser , name , cost))
            data.commit()
            QtWidgets.QMessageBox.information(None, "Success", "Data has been saved")
            self.warning(False)
    ...
    def warning(self , checked = False):
        try:
            ...
            if savings:
                ...
                cursor.execute("SELECT SUM(cost) AS Total_cost FROM savings WHERE iduser = %s", (self.iduser,))
    ...
    def retranslateUi(self, Form):
        ...
        self.pushButton.setText(_translate("Form", "1 millon"))
        self.pushButton_2.setText(_translate("Form", "10 millon"))
        self.pushButton_3.setText(_translate("Form", "100 million"))
